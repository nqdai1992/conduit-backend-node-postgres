...
export default function SignUpController (signUpUseCase: SignUpControllerInputPort) {
  const router = Router();
  const path = '/api/users'


  router.post(path, async (req: AppRequest<{
    email: string,
    password: string,
    username: string
  }>, res) => {
    try {
      ...
      const userIsExisted = await signUpUseCase.userIsExisted(req.body.username, req.body.email)
      ...
      const user = await signUpUseCase.registerUser(req.body.username, req.body.email, req.body.password)
